...
async function investigate() {
  console.log('🔍 INVESTIGAÇÃO MINIPREÇO\n')
  ...
  const url = 'https://www.minipreco.pt/search?q=massa+esparguete'
  console.log(`📍 ${url}\n`)
  ...
  const info = await page.evaluate(() => {
    ...
    // Procurar todos os elementos que possam ser produtos
    const candidates = [
    ...
    unique.slice(0, 20).forEach((el, i) => {
      ...
      // Procurar nome
      const nameEl = el.querySelector('h1, h2, h3, h4, [class*="name"], [class*="title"], [class*="Name"], [class*="Title"]')
